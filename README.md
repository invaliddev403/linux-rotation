...
Python 3 must already be installed (package `python3` on Debian/Fedora/openSUSE, `python` on Arch). The script uses only Python's standard library.
...
For `watch`, stop the process; its initial display rotation and optional X11 touch matrix are restored. Undo does not uninstall packages or stop the shared sensor service. No bootloader, login-screen, kernel, udev, sensor mount matrix, or tablet-mode driver changes are made.
